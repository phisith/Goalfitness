...
import com.example.goalfitness.Start;
...
public class CommentAdapter extends RecyclerView.Adapter<CommentAdapter.ImageViewHolder> {
    ...
    private void getUserInfo(final ImageView imageView, final TextView username, String publisherid){
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference()
                .child("Users").child("Customers").child(publisherid);

        reference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                Customer_Register customer_register = dataSnapshot.getValue(Customer_Register.class);
                Glide.with(mContext).load(customer_register.getImageURL()).into(imageView);
